...
# Setup shaderc and its dependencies for iOS builds
...
# Copy iOS-specific wraps (shaderc and dependencies)
echo "Copying iOS shaderc wraps..."
for wrap in shaderc glslang SPIRV-Tools SPIRV-Headers; do
  if [ -f "../../wraps/ios/${wrap}.wrap" ]; then
    cp "../../wraps/ios/${wrap}.wrap" subprojects/
  fi
done
